...
if rg -n 'Invoke-Expression|iex[[:space:]]|Start-Process.*-Verb[[:space:]]+RunAs' \
...
if command -v pwsh >/dev/null 2>&1; then
  parse_file() {
    ...
    pwsh -NoLogo -NoProfile -Command \
      "& { \$tokens = \$null; \$parseErrors = \$null; [System.Management.Automation.Language.Parser]::ParseFile('$file',[ref]\$tokens,[ref]\$parseErrors) | Out-Null; if (\$parseErrors.Count -gt 0) { exit 1 } }"
  }
  ...
  if parse_file "$TMP_DIR/invalid.ps1"; then
